...
app.post('/startAll', (req, res) => {
    try {
        containers.startAllContainers();
        res.send({status: 'ok', message: 'containers started'});
    } catch(error) {
        res.status(500).send({status: 'error', message: 'Internal server error'});
        console.error(error);
    }
...
app.post('/stopAll', (req, res) => {
    try {
        containers.stopAllContainers(req.body.name);
        res.send({status: 'ok', message: 'containers stopped'});
    } catch(error) {
        res.status(500).send({status: 'error', message: 'Internal server error'});
        console.error(error);
    }
